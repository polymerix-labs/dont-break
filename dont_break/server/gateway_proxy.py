...
async def proxy_api_request(
    ...
    *,
    ...
    if kind == "query":
        path = GatewayRoutes.query(workspace, project_key, suffix)
    elif kind == "assist":
        path = GatewayRoutes.assist(workspace, project_key, suffix)
    else:
        path = GatewayRoutes.rules(workspace, project_key, suffix)
